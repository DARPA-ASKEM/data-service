...
@workflow_router.get("", **retrieve.fastapi_endpoint_config)
def list_workflows(page_size: int = 100, page: int = 0) -> JSONResponse:
    """
    Retrieve the list of workflows from ES.
    """
    es = es_client()
    list_body = {"size": page_size, "source": False, "from_": page}
...
@workflow_router.get("/workflow_id}", **retrieve.fastapi_endpoint_config)
def workflow_get(workflow_id: str | int) -> JSONResponse | Response:
    """
...
def workflow_put(workflow_id: str | int, payload: Workflow) -> JSONResponse | Response:
    """
    Update a workflow in ElasticSearch
    """
    try:
        res = payload.save(workflow_id)
...
def workflow_delete(workflow_id: str | int) -> Response:
    """
